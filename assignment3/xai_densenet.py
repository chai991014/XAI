...
def calculate_metrics(model, image_tensor, heatmap, device):
    """Calculates Confidence Drop (using 50% mask) and RRS metrics."""
    # Using 50th percentile to force a meaningful change in model confidence
    ...
    # RRS = Average relevance of top pixels
    rrs = np.mean(heatmap[heatmap >= threshold])
    return conf_drop, rrs


def run_xai(MODEL_PATH, XAI_OUTPUT_DIR, DEVICE):
    # 1. Load and Patch Model
    model = DenseNet().to(DEVICE)
    model = disable_inplace(model)  # Ensure gradient stability
    model.load_state_dict(torch.load(MODEL_PATH, map_location=DEVICE), strict=False)

    model.cbam.fc_max.load_state_dict(model.cbam.fc.state_dict())
    model.eval()
    ...
    # Targeting final normalization layer of DenseNet121
    ...
    for img_id in tqdm(range(len(test_dataset))):
        ...
        # A. Grad-CAM++ (Targeting index 0)
        model.zero_grad()
        gcam_out = model(input_tensor)
        gcam_raw = cam_extractor(0, gcam_out)[0]

        # Reshape to 4D [1, 1, 7, 7] for interpolation
        if gcam_raw.ndim == 3:
        ...
        c_drop, c_rrs = calculate_metrics(model, input_tensor, crise_norm, DEVICE)

        metrics_list.append({
            "Image_ID": img_id, "Category": cat_name, "True": true_label, "Pred": pred_label,
            "GCAM_Drop": round(g_drop, 4), "GCAM_RRS": round(g_rrs, 4),
            "C-RISE_Drop": round(c_drop, 4), "C-RISE_RRS": round(c_rrs, 4)
        })
        ...
        plt.subplot(1, 3, 3)
        plt.title(f"C-RISE (Drop: {c_drop:.2f} | RRS: {c_rrs:.2f})")
        plt.imshow(np.clip(img_vis, 0, 1))
        ...
        # Re-initialize hooks for the next image
        cam_extractor = GradCAMpp(model, target_layer=target_layer)

    pd.DataFrame(metrics_list).to_csv(os.path.join(XAI_OUTPUT_DIR, "xai_metrics_subset.csv"), index=False)
